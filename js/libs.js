...
(function($) {
    ...
    jQuery.fn.rater = function(options) {
        ...
        return this.each(function() {
            ...
            opts.size = $on.height() - (opts.scale*2);
            if (typeof(opts.rating) === 'undefined') {opts.rating = ($on.width() - opts.scale) / opts.size;}
            if (typeof(opts.id) === 'undefined') {opts.id = $this.attr('data-id');}

            $off.mousemove(function(e) {
                var left = e.clientX - $off.offset().left;
                var width = ($off.width() - opts.scale) - ($off.width() - opts.scale - left);
                width = Math.ceil(width / (opts.size / opts.step)) * opts.size / opts.step;
                $on.width(width + (opts.scale/2));
                ...
                complete: function(req) {
                    if (req.status === 200) { //success
                        ...
                        $off.fadeTo(600, 0.1, function() {
                            ...
                            //var $count = $this.find('.ui-rater-rate-count');
                            //$count.text(parseInt($count.text()) + 1);
                            //$this.find('.ui-rater-rating').text(opts.rating.toFixed(1) + 5);
                            $off.fadeTo(600, 1);
...
l*(b[0].scrollHeight-b.outerHeight());e&&(g=d,d=g/b[0].scrollHeight*b.outerHeight(),d=Math.min(Math.max(d,0),f),c.css({top:d+"px"}));b.scrollTop(g);b.trigger("slimscrolling",~~g);w();p()}function x(){u=Math.max(115);c.css({height:u+"px"});var a=u==b.outerHeight()?"none":"block";c.css({display:a})}function w(){x();clearTimeout(B);l==~~l?(k=a.allowPageScroll,C!=l&&b.trigger("slimscroll",0==~~l?"top":"bottom")):k=!1;C=l;u>=b.outerHeight()?k=!0:(c.stop(!0,
